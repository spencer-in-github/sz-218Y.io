...
Redwood City has 105 sensors. San Mateo City has 108 sensors.
```{r rwc&smc-sensors}
...
For San Mateo City, most of the inside sensors only has data for Channel A, so we skip the difference comparison step and look at the distribution of observed data in Channel A alone. There are no anomoly in the observed data.
...
The four faulty sensors excluded from the following analysis is shown in Map X.
...
## Project voronoi to blocks/CBGs/parcels
```{r download-smc-cbgs}
library(tigris)
